...
import { resolveCollection } from "./utlis/collections.js";
import * as _ from "./utlis/utils.js";
...
export default class IconPicker {
	...
	_preBuild() {
		...
		if (
			!Array.isArray(this.options.iconSource) &&
			this.options.iconSource.length > 0
		) {
	...
	_binEvents() {
			...
			_.addEvent(
				...
				_.debounce((evt) => {
					const iconsResult = this.availableIcons.filter((obj) => {
						return (
							obj.value.includes(evt.target.value.toLowerCase()) ||
							obj.categories?.filter((c) =>
								c.includes(evt.target.value.toLowerCase()),
							).length > 0
						);
					});
					...
					if (iconsResult.length > 0) {
						...
						if (!emptyElement) {
							root.content.appendChild(
								_.stringToHTML(
									`<div class="is-empty">${options.i18n["text:empty"]}</div>`,
								),
							);
						}
					}
		...
		if (!options.closeOnSelect) {
			this._eventBindings.push(
				_.addEvent(root.save, "click", () => this._onSave()),
			);
		}
	}
	...
	async _renderdIcons() {
		...
		const icons = await this._getIcons();

		icons.forEach((library) => {
			const iconFormat = library.iconFormat ? library.iconFormat : "svg";
			...
			for (const [key, value] of Object.entries(library.icons)) {
				...
				if (
					library.categories &&
					Object.entries(library.categories).length > 0
				) {
					categories = [];
				...
				if (iconFormat === "i" || !value.body) {
					...
					iconElement.setAttribute(
						"viewBox",
						`0 0 ${value.width ? value.width : library.width} ${value.height ? value.height : library.height}`,
					);
				...
				iconTarget.addEventListener("click", (evt) => {
					if (
						this.currentlySelectName !== evt.currentTarget.firstChild.className
					) {
		...
		if (options.defaultValue || this.element.value) {
			...
			const defaultValueElement = document.querySelector(
				`[data-value="${options.defaultValue ? options.defaultValue : this.element.value}"]`,
			)
				? document.querySelector(
						`[data-value="${options.defaultValue ? options.defaultValue : this.element.value}"]`,
					)
				: document.querySelector(
						`.${options.defaultValue ? options.defaultValue : this.element.value}`,
					);

			const iconValue = defaultValueElement?.dataset.value ?? "";
	...
	async _getIcons() {
		...
		const sourceInformation = resolveCollection(options.iconSource);
		...
		return await Promise.all(
			iconsURL.map((iconURL) =>
				fetch(iconURL).then((response) => response.json()),
			),
		).then((iconLibrary) => {
